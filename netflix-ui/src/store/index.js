...
export const getUsersLikedMovies = createAsyncThunk(
  ...
  async (email) => {
    const {
      data: { movies },
    } = await axios.get(`https://netflix-clone-backend-txsj.onrender.com/api/user/liked/${email}`);
    return movies;
  }
);
...
export const removeMovieFromLiked = createAsyncThunk(
  ...
  async ({ movieId, email }) => {
    const {
      data: { movies },
    } = await axios.put("https://netflix-clone-backend-txsj.onrender.com/api/user/remove", {
      email,
      movieId,
    });
    return movies;
